"""
win_launcher.py — Windows System Tray Status Wrapper for TubeLM.

Runs Flask in a background daemon thread and places a premium native icon
in the Windows Taskbar System Tray to Open Dashboard, trigger Sync pipelines, or Exit.
"""
...
logger = logging.getLogger("TubeLM-WinLauncher")
...
def trigger_sync(icon, item):
    ...
    # Send desktop notification on Windows via pystray native notify
    try:
        icon.notify(
        ...
        logger.warning("Failed to trigger Windows notification: %s", e)
...
def main():
    ...
    logger.info("Starting native Windows System Tray Icon on port %d...", PORT)
