...
<head>
  ...
  <title>TubeLM - Executive Briefing (Full 3-Tab Experience)</title>
  ...
  <style>
    * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; margin: 0; padding: 0; }}
    ...
    .card.read {{ opacity: 0.5; filter: grayscale(0.5); }}
    ...
    .sync-sub {{ font-size: 11px; color: var(--text-muted); }}
...
<body>
  <div class="phone">
    ...
    <div class="view-content" id="mainContainer">
      ...
      <!-- ========================================== -->
      <!-- TAB 1: BRIEFING CONTENT (DEFAULT)          -->
      <!-- ========================================== -->
        ...
        <div class="card" id="card-{it['rank']}">
          ...
          <div class="card-actions">
            ...
            <div class="action-icons">
              <button class="action-btn" id="btn-save-{it['rank']}" onclick="toggleSaveItem({it['rank']}, '{html.escape(it['title'])}', '{html.escape(it['source_name'])}')">
...
for it in next10:
    mock1_html += f"""
        <div class="card" id="card-{it['rank']}">
          ...
          <div class="card-actions">
            <button class="btn-audio" style="padding:5px 12px; font-size:11px;" onclick="playAudio('{html.escape(it['title'])}', '{html.escape(it['source_name'])}')">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg> Audio
            </button>
            <div class="action-icons">
              <button class="action-btn" id="btn-save-{it['rank']}" onclick="toggleSaveItem({it['rank']}, '{html.escape(it['title'])}', '{html.escape(it['source_name'])}')">Save</button>
      ...
      <div id="tab-channels-view" style="display:none;">
        <div class="channels-header">
          ...
          <div class="cat-scroll">
            <div class="cat-pill active" onclick="selectChannelCat(this, 'all')">All Channels ({len(channels)})</div>
            <div class="cat-pill" onclick="selectChannelCat(this, 'tech')">Tech & AI ({len(categories['tech'])})</div>
            <div class="cat-pill" onclick="selectChannelCat(this, 'health')">Health & Bio ({len(categories['health'])})</div>
            <div class="cat-pill" onclick="selectChannelCat(this, 'deep_explainer')">Science & Deep ({len(categories['deep_explainer'])})</div>
          </div>
        </div>
...
for ch in channels:
    ...
    mock1_html += f"""
          <div class="channel-card" data-category="{ch.get('category')}" data-name="{ch.get('name', '').lower()}">
            <div class="ch-top">
              <div class="ch-avatar">{initials}</div>
              <div class="ch-meta">
                <div class="ch-name">{html.escape(ch.get('name'))}</div>
                <div class="ch-subline">
                  <span style="font-weight:700; text-transform:uppercase; color:var(--accent);">{ch.get('category')}</span>
                  · <span>{ch.get('video_count', 1)} briefing{'s' if ch.get('video_count', 1) > 1 else ''}</span>
            ...
            <div class="ch-actions">
              <button class="btn-audio" onclick="playAudio('Channel Digest: {html.escape(ch.get('name'))}', '{html.escape(ch.get('category', '').upper())}')">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                Listen Summary
              </button>
...
mock1_html += f"""
      ...
      <div id="tab-saved-view" style="display:none;">
        ...
        <div class="sync-card">
          <div>
            ...
            <div class="sync-sub">Synced with tubelm-sync.workers.dev (Laptop & App)</div>
          </div>
          <button class="action-btn" onclick="forceSync()" style="color:var(--accent);">Sync Now</button>
        </div>
        ...
        <div id="saved-items-container">
          <div class="card" id="saved-card-1">
            <div class="card-top">
              ...
              <div class="meta-line"><span>{html.escape(top10[0]['source_name'])}</span> · <span>{top10[0]['duration']}</span></div>
            </div>
            <div class="card-title">{html.escape(top10[0]['title'])}</div>
            <div class="why-box">
              <div class="why-label">Why It Matters</div>
              <div class="why-text">{html.escape(top10[0]['why_it_matters'])}</div>
            </div>
            <div class="card-actions">
              <button class="btn-audio" onclick="playAudio('{html.escape(top10[0]['title'])}', '{html.escape(top10[0]['source_name'])}')">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                Play Audio
              </button>
          ...
          <div class="card" id="saved-card-2">
            <div class="card-top">
              ...
              <div class="meta-line"><span>{html.escape(top10[1]['source_name'])}</span> · <span>{top10[1]['duration']}</span></div>
            </div>
            <div class="card-title">{html.escape(top10[1]['title'])}</div>
            <div class="why-box">
              <div class="why-label">Why It Matters</div>
              <div class="why-text">{html.escape(top10[1]['why_it_matters'])}</div>
            </div>
            <div class="card-actions">
              <button class="btn-audio" onclick="playAudio('{html.escape(top10[1]['title'])}', '{html.escape(top10[1]['source_name'])}')">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                Play Audio
              </button>
  ...
  <script>
    ...
    function forceSync() {
      alert('Cloudflare Worker: Synced read states and commute queue with R2 bucket!');
    }
  </script>
</body>
</html>
...
with open(".workflow/mocks/mock1_executive_briefing.html", "w", encoding="utf-8") as f:
    f.write(mock1_html)
print("Updated mock1_executive_briefing.html with full 3-tab experience and Light Mode support!")
